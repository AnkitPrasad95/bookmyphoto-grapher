			...
			<div class="col-auto float-right ml-auto breadcrumb-menu align-self-center">
				<nav aria-label="breadcrumb" class="page-breadcrumb">
					<ol class="breadcrumb">
						...
						<li class="breadcrumb-item active" aria-current="page">{{ trans('labels.handyman') }}</li>
		...
		<div class="row">
							...
							<tr>
								<td>
									<div class="custom-control custom-radio" onclick="closeaddressform();">
										...
										<input class="custom-control-input" id="{{$address->id}}" name="address" data-fullname="{{$address->name}}" data-email="{{$address->email}}" data-mobile="{{$address->mobile}}" data-street_address="{{$address->street}}" data-landmark="{{$address->landmark}}" data-postcode="{{$address->postcode}}" type="radio">
			...
			<div class="col-12 col-md-12 col-lg-4">
				...
				<div class="cart_detail_box mb-4 flex" style="display: inline-flex;">
					...
					<input type="checkbox" class="custom-control custom-radio" checked name="instant_booking" id="instant_booking" value="1" style="margin-left: 17px;margin-top: -9px;width: 1rem;">
				</div>

				<div class="cart_detail_box mb-4">
					<input type="date" class="form-control form-control-sm" name="date" id="date" min="<?= date('Y-m-d') ?>" value="<?= date('Y-m-d') ?>">
					<input type="time" class="form-control form-control-sm" name="time" id="time" value="<?= date('H:i'); ?>">
				</div>
					...
					<ul class="list-group list-group-sm list-group-flush-y list-group-flush-x">
						<li class="list-group-item d-flex">
							<span>{{trans('labels.price')}}</span> <span class="ml-auto font-size-sm">
								{{Helper::currency_format(Storage::disk('local')->get('ins_price'))}}
							</span>
						</li>
						<li class="list-group-item d-flex">
							<span>{{trans('labels.discount')}}</span> <span class="ml-auto font-size-sm">
								{{Helper::currency_format(Storage::disk('local')->get('ins_total_discount'))}}
							</span>
						</li>
						<li class="list-group-item d-flex font-size-lg font-weight-bold">
							<span>{{trans('labels.total')}}</span> <span class="ml-auto font-size-sm">
								{{Helper::currency_format(Storage::disk('local')->get('ins_total_price'))}}
							</span>
						</li>

						<input type="hidden" name="price" id="price" value="{{$photographerData['booking_price']}}">
						<input type="hidden" name="total_price" id="total_price" value="{{Storage::disk('local')->get('ins_total_price')}}">
						<input type="hidden" name="discount" id="discount" value="{{Storage::disk('local')->get('ins_total_discount')}}">
						<input type="hidden" name="service" id="service" value="ptotographer_{{ $photographerData['photographer_id'] }}">
						...
						<input type="hidden" name="coupon_code" id="coupon_code" value="{{Storage::disk('local')->get('ins_coupon_code')}}">
						<input type="hidden" name="photographer_id" id="photographer_id" value="{{Storage::disk('local')->get('photographer_id')}}">
						@endif
					</ul>
				</div>
